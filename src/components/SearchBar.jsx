...
export const SearchBar = ({ data, onSelectApartment, onSelectFloor }) => {
    ...
    data.floors.forEach(floor => {
      ...
      floor.apartments.forEach(apt => {
        ...
        apt.residents.forEach(res => {
          ...
          // Buscar en Vehículos y Patentes
          ...
          {suggestions.map((result) => (
            ...
            >
              <div className="min-w-0">
                <div className="text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">{result.label}</div>
                <div className="text-[10px] text-slate-400 uppercase font-bold truncate">{result.sublabel}</div>
              </div>
              <span className="shrink-0 text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-md font-bold uppercase group-hover:bg-slate-900 dark:group-hover:bg-white dark:group-hover:text-slate-900 group-hover:text-white transition-colors ml-2">
                {result.type}
              </span>
            </button>
          ))}
